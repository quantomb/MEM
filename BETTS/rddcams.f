...
c       Declare local variables
        character*72 linemc,string1

        integer, parameter :: nbinsmax=20000
        integer :: i,j,ic,jc,k,l,lr,i1,n,nuse,ind,naver,ihisto,
     &             nbins,iopt,nbinsuse,nbinsuse_per_proc,
     &             info,isign,idecimate,
...
c        real(kind), parameter :: signswitch=0.1_kind
     ...
     &                betal,edl,Ul,tprimel,smeas,signa,fillsbin(nbinsmax),
...
c       Get the output data filename
        ...
        write(string1,"('tau',a20)") linemc
        open(unit=991,file=string1,status='unknown')
...
c       iopt=7, read the cluster anomalous single-particle Green function F 
c*******************************************************************************
...
c
        ...
        read(5,*) naver
        idecimate=0 
        if(naver.lt.0) then  ! if idecimate=1, we drop every other data point
          idecimate=1
          naver=-naver
        end if
...
c         READ IN THE HEADER DATA (NOTE THE ASSUMED FORM)
 400      read(92,"(a72)") linemc
          if(index(linemc(1:72),'Results').ne.0 ) then
            read(92,*) nl,nwn,cluster,ndim
            nuse=nl    ! number of time slices used in MEM, cond. reset below
          else
           goto 400
          endif
...
c*******************************************************************************
c         Read in all the lines containing Gtau, and try to 
...
c            if(index(linemc(1:72),'bin of  Chitau').ne.0) then
              i1=i1+1
              read(linemc,"(35x,i15)") proc
              if(proc.gt.nproc) nproc=proc
c         write(*,*) "i1,proc=",i1,proc
              read(92,*) ((gsmeas(j,icr), j=1,nl),icr=1,Nc),smeas 
c         write(*,*) "gsmeas(1,1)=",gsmeas(1,1)
              ave=ave+smeas
            end if
          end do
 444      nproc=nproc+1
...
c*******************************************************************************
c         Now read physical parameters from the binned data files
c*******************************************************************************
...
c***************************************************************************
          if(iopt.eq.7) then
c***************************************************************************
c           Create symmetry factors used for the anomalous part of the Green
c           function (iopt=7).
c***************************************************************************
            if(u.lt.zeror) then ! U<0 and pairing symmetry same as lattice
              symfack=oner
              symfacr=oner
            else if(u.gt.zeror) then
              do ic=1,Nc
                if(abs(Kc(1,ic)**2 - Kc(2,ic)**2).lt.eps) then
                   symfack(ic,:)=zeror
                else
                  do iequiv=1,ngroup
                    jc=ickequ(ic,iequiv)
                    symfack(ic,jc)=(Kc(1,jc)**2-Kc(2,jc)**2)
     &                            /(Kc(1,ic)**2-Kc(2,ic)**2)
                  end do
                end if 
              end do
              do ic=1,Nc
                if(abs(Rc(1,ic)**2 - Rc(2,ic)**2).lt.eps) then
                   symfacr(ic,:)=zeror
                else
                  do iequiv=1,ngroup
                    jc=icrequ(ic,iequiv)
                    symfacr(ic,jc)=(Rc(1,jc)**2-Rc(2,jc)**2)
     &                            /(Rc(1,ic)**2-Rc(2,ic)**2)
                  end do
                end if 
              end do
...
c***************************************************************************
c
          if(abs(ed/U).lt.0.0001.and.abs(tprime).lt.0.0001
     &    .and.abs(tdprime).lt.0.0001.and.abs(t3prime).lt.0.0001
     &    .and.(ick.eq.0.or.ickmap(ickdiff(ickpi,max(1,ick))).eq.ick)
     &    .or.iopt.ge.1)then
c           Use symmetric storage mode if G(tau)=G(beta-tau) or if
c           G(tau)=-G(beta-tau) (i.e. when iopt=7)
            write(6,*) 'Symmetric storage mode used'
            write(9,*) 'symmetric storage mode used'
            if(iopt.eq.7) then
              nuse=nuse/2 ! since G_12(tau=beta/2)==0 and G_12(tau)=-G_12(beta-tau)
            else
              nuse=nuse/2+1 ! due to symmetry, G(tau)=G(beta-tau)
            end if
          end if
...
c          ibinn=ibin  ! note ibin its needed below
...
c             find a bin header
 409          read(92,"(a72)",end=16) linemc
            if(index(linemc(1:72),'bin of  Gtau').ne.0) then
c              if(index(linemc(1:72),'bin of  Chitau').ne.0)
c     &        then
...
c                if(proc.eq.iproc) then !readin a bin
c                  ibinn=ibinn+1        ! doesn't start at 0 if ifile>1
                  read(92,*) ((gsmeas(j,icr), j=1,nl),icr=1,Nc),smeas 
                  sbin(ibinn)=smeas
                  gsmeas=-gsmeas ! delete this line later
...
c                end if
 ...
 16         continue
            rewind(92)
c            write(6,*) 'done first read for proc',iproc
c          end do
          rewind(92)
...
c             find a bin header
 408          read(92,"(a72)",end=15) linemc
     ...
     &           index(linemc(1:72),'bin of  Ftau').ne.0.and.iopt.eq.7.or.
...
c                   Now symmetrize the Green function data
                    ...
                    if(iopt.ne.7) then
                      do n=1,nl                                           
                        do icr=1,Nc       
                          do iequiv=1,ngroup                               
                            jc=icrequ(icr,iequiv)
                            data1(n,icr)=data1(n,icr) + gsmeas(n,jc) 
                          end do                                  
                        end do                                              
                      end do    
                    else if(iopt.eq.7) then
c                     Impose the symmetry F(tau) = -F(beta-tau), which
c                     comes from F(-tau)=F(tau), and the s-wave or d-wave
c                     symmetry of the order parameter
                      do n=2,nl                                           
                        do icr=1,Nc       
                          do iequiv=1,ngroup                               
                            jc=icrequ(icr,iequiv)
                            data1(n,icr)=data1(n,icr)+ symfacr(icr,jc)*
     &                                   halfr*(gsmeas(n,jc)-gsmeas(nl-n+2,jc)) 
                          end do                                  
                        end do                                              
                      end do    
                      do icr=1,Nc       
                        do iequiv=1,ngroup                               
                          jc=icrequ(icr,iequiv)
                          data1(1,icr)=data1(1,icr)+ 
     &                                 symfacr(icr,jc)*gsmeas(1,jc)
                        end do                                  
                      end do                                              
                    end if
...
cc         For susceptibilities in particle-particle channels, sysmetrize 
cc         along the imaginary time direction to guarantee the boson type 
cc         operator property: \chi_p(\tau)=\chi_p(\beta-\tau)
cc         suggested modification made by Shi-Quan Su 2009/10/23
c                if (iopt.eq.4)then 
cc         iopt=4 is pairing susceptibility channel, is a particle-particle channel.
c                  do n=2,nl/2
c                    ds(n,ibin)=0.5*(ds(n,ibin)+ds(nl-n+2,ibin))
c                    ds(nl-n+2,ibin)=ds(n,ibin)
c                  enddo
c                endif                 
...
c            rewind(92)
c            write(6,*) 'done second read for proc',iproc
c          end do
...
c                                                     ! <s>/<<s>>   for l=nuse+1
        else if(iopt.ge.1.and.iopt.ne.7) then
           sus=0.0
           r1=2.0/3.0
           do l=1,nl
...
c          is one.
           write(17,"(2x,i4,3x,f7.3,3x,f11.4,3x,f11.4)") ick,beta,sus/sus1,sus2/sus1   ! ick, beta, chi(T), chi(tau=0)
        end if 
...
c       divided by the global average sign <<s>>, so the ratio is precisely 
c       what we want; e.g.
c
c       <Gs>/<s> when iopt=0 or 7.
c       or
...
c*******************************************************************************
        if(isign.eq.1) then
          do i=1,nbins
             ...
             do k=1,nl
c               data(nuse+1,i) = <s>/<<s>>, and data(k<nuse,i) = <Gs>/<<s>>
c               so the line below creates <Gs>/<s>.
...
c       The relationship between nuse, nl and isign is presently
c
c                          { nl/2 +1    iopt = 7                where  C = [ <<dG dG>>  <<dS dG>> ]
c       If isign=0, nuse = { nl/2 +2    symmetric storage mode             [ <<dG dS>>  <<dSdS>>  ]
c                          { nl +1	else                      and <> indicates an average over bins
c
c                          { nl/2	iopt = 7                where C = <<dG'dG'>>
c       If isign=1, nuse = { nl/2 + 1 	symmetric storage mode  and G'= < GS>/<SL 
c                          { nl		else                    where <> indicates an average in each bin
c
c       If idecimate =0, these numbers should remain.  If idecimate =1, they should become
c
c                          { nl/4 +1    iopt = 7                where  C = [ <<dG dG>>  <<dS dG>> ]
c       If isign=0, nuse = { nl/4 +2    symmetric storage mode             [ <<dG dS>>  <<dSdS>>  ]
c                          { nl/2 +1	else                      and <> indicates an average over bins
c
c                          { nl/4	iopt = 7                where C = <<dG'dG'>>
c       If isign=1, nuse = { nl/4 + 1 	symmetric storage mode  and G'= < GS>/<SL 
c                          { nl/2	else                    where <> indicates an average in each bin
c
...
c       Then we reject the data for which the probability falls below Pmin.
        lr=0
        do l=1,nuse-(1-isign)   ! exclude the sign l
c          if(l.ne.1.and.Aprob(l).gt.Pmin.or.l.eq.nuse.and.iopt.ne.7) then
c          keep this set of data (always throw away \tau=0 point)

           if(Aprob(l).gt.Pmin.or.l.eq.1.or.l.eq.nuse.and.iopt.ne.7) then  !!origin
c           Keep this set of data unless idecimate=1, in which case we reject even l
            if(idecimate.eq.1.and.mod(l,2).eq.0) cycle
            lr=lr+1
            datar(lr,:)=data(l,:)
            Ataur(lr)=Atau(l)
            averr(lr)=aver(l)
        ...
        if(isign.eq.0) then ! treat the sign special in location l=nuse
          lr=lr+1 
          datar(lr,:)=data(nuse,:)
          Ataur(lr)=Atau(nuse)
          averr(lr)=aver(nuse)
...
c23456789012345678901234567890123456789012345678901234567890123456789012
     ...
     &             ' prob'
        ...
        do l=1,nuse
          write(9,"(1x,f9.4,1x,e14.7,1x,e14.7,1x,f5.2,1x,f5.2,1x,f7.4)") 
     &         Atau(l),aver(l),Aerror(l),Acurt(l),Askew(l),Aprob(l)
          write(991,"(1x,f9.4,1x,e14.7,1x,e14.7,1x,f5.2,1x,f5.2,1x,f7.4)") 
     &         Atau(l),aver(l),Aerror(l),Acurt(l),Askew(l),Aprob(l)
        end do
