...
c*******************************************************************************
        ...
        implicit none
c*******************************************************************************
        character*72 linemc,string1,string2, string3,string4
        
        integer, parameter :: itermax=1000  ! max number of iterations
        
        integer :: i,j,k,l,n,m,p,ick,icq,ic,icp,iequiv,info,
     &             run,meas,minbound,maxbound,ix,iy,iter,nordpref,
     &             unit,minp,ios

        real(kind) :: test,testl,tolerance,kx,ky,kz,dnded,dndedl,edl,nfoccl,
     &                nfocc,Epsk,r1,r2,damp,damp1,xsc
...
c*******************************************************************************
        ...
        if(ndim.eq.2) then
          ...
          do ick=1,Ncw
            write(16,"(1x,i3,1x,f9.6,2x,f9.6,4x,i3)") ick,Kc(1,ick),Kc(2,ick),ickdeg(ick)
          end do
          write(16,*) '  '
          do ick=Ncw+1,Nc
...
c           test for convergence by inspecting Im[sigma(0,ick) ]
            ...
            r1=abs(testl-test)
            write(6,"(' test= ',e11.4,'  ed=',f8.5,'  nf=',f8.5,
...
c       ..like write it out
        ...
        do ic=1,Nc
        do n=0,nwn
          write(33,"(1x,i3,1x,i3,2x,e13.6,1x,e13.6,2x,e13.6,1x,e13.6)") n,ic,sigma(n,ic),chi(n,ic)
        end do
        end do
c*******************************************************************************
...
c           Open some files.
            if(ick.lt.10) then
              ...
              write(string3,"('model_sigkw',i1)") ick
              write(string4,"('model_Cumukw',i1)") ick
            else
              write(string1,"('model_Akw',i2)") ick
              write(string2,"('Sigma_pt',i2)") ick
              write(string3,"('model_sigkw',i2)") ick
              write(string4,"('model_Cumukw',i1)") ick
            end if
            open(unit=59,file=string1,status='unknown') !model_Akw_#
            open(unit=60,file=string3,status='unknown') !model_sigkw_#
            open(unit=61,file=string4,status='unknown') !model_Cumukw_#
...
c              do n=1,min0(minp,nwn/2)
              do n=1,5!delete it later
                write(6,*) n,tau(n)
                write(10,*) n,tau(n)
              end do
              write(6,*) '  '
...
c*******************************************************************************
        ...
        implicit none
c*******************************************************************************
        integer, parameter :: nf=300  
c nf should small than the nfmax defined in dsigcluster.f
        integer :: nchi,n,n1,i,j,nordpref,unit,iKx,iKy,ick

        real(kind), parameter :: wmax=10.0_kind, delta=0.8_kind
        real(kind) :: tau(*),r1,r2,Escale,r3,r4,y,Epsk,kx,ky,kz,
     &                w(nf),dw(nf), sigmanorm
...
c*******************************************************************************
        ...
        do n=1,nchi   ! n is the order of the Pade'.
         ...
         if(unit.eq.61) then !Cumulant M = 1/(iwn-ed-Sig)
         Epsk=0.0
         write(*,*) "unit=",unit
         else if((unit.ne.69).and.(unit.ne.61)) then
...
c         and write the result into file unit.
         ...
         if(unit.eq.61) then !Cumulant M = 1/(iwn-ed-Sig)
         write(unit,"('#     w        dw   Re[M(w)] Im[M(w)] ImM(K,w)/(-pi)')") 
         else if((unit.ne.69).and.(unit.ne.61)) then
         write(unit,"('#     w        dw   Re[sigma(w)] Im[sigma(w)] A(K,w)')") 
         end if
...
c           form the approximant.
            ...
            c1=apade(0)/c1
            Sigma(n)=ii*c1
            sigmanorm = sigmanorm + dw(n)*aimag(Sigma(n))
            if(abs(w(n)).lt.wmax.and.unit.ne.69) then
              ...
              write(unit,"(f10.6,1x,f10.6,1x,f10.6,1x,f10.6,1x,f10.6)") w(n),dw(n),r3,r2,r4
            end if
          end do

c         Write the normalize (to one) imaginary part of the self energy to a file
          do n=1,nf
            if(abs(w(n)).lt.wmax.and.unit.ne.69) then
              write(unit+41,"(e20.10,1x,e20.10,1x,e20.10)") w(n),aimag(Sigma(n))/(sigmanorm),dw(n)
            end if
          end do
     ...
     &                  1.0_kind/(w(n) - Epsk - ed - Sigma(n))
            ...
            do n=1,nf
              GcKf(n)=GcKf(n)/real(nwsc)
                Cumu(n)=Cumu(n)+
     &                  1.0_kind/(w(n) -  ed - Sigma(n))
              r3=-aimag(GcKf(n))/pi
              if(r3.lt.0.0000001_kind) r3=0.0000001_kind
              if(abs(w(n)).lt.wmax) write(unit+40,"(e20.10,1x,e20.10,1x,e20.10)") w(n),r3,dw(n)
              r3=-aimag(Cumu(n))/pi
              if(r3.lt.0.0000001_kind) r3=0.0000001_kind
              if(abs(w(n)).lt.wmax) write(unit+42,"(e20.10,1x,e20.10,1x,e20.10)") w(n),r3,dw(n)
            end do
          else
            ...
            do n=1,nf
              r3=-aimag(Sigma(n))/pi
              if(r3.lt.0.0000001_kind) r3=0.0000001_kind
              if(abs(w(n)).lt.wmax) write(unit,"(e20.10,2x,e20.10,2x,e20.10)") w(n),r3,dw(n)
            end do
          end if
        end if
