...
c*******************************************************************************
	...
	integer, parameter :: nover=100
     ...
     &              ios
        ...
        real(kind) :: r1, r2, r3, kx, ky,  Epsk, kx1, ky1, kx2, 
     &                ky2, offset, res, hall, vx, vyy, dfermi
...
c          allocate (g1_sp(-N_sp:N_sp),stat=info)
c          infot=info
c          allocate (g2_sp(-N_sp:N_sp),stat=info)
c          infot=infot+info
c          allocate (Sig_sp_r(-N_sp:N_sp,-N_sp:N_sp,nf),stat=info)
c          infot=infot+info
c          allocate (Derivs_sp_r(-N_sp:N_sp,-N_sp:N_sp,nf),stat=info)
c          infot=infot+info
c          allocate (Sig_sp_i(-N_sp:N_sp,-N_sp:N_sp,nf),stat=info)
c          infot=infot+info
c          allocate (Derivs_sp_i(-N_sp:N_sp,-N_sp:N_sp,nf),stat=info)
c          infot=infot+info
c          if(infot.ne.0) write(6,*) 'allocate error,infot=',infot
...
c         1. form the grid in the space of g1 and g2
          ...
          do n=1,nf
            ...
            do j=-N_sp,N_sp
              ick=ict(i,j,0)              
c              Sig_sp_r(i,j,n)=real(Sigma(n,ick))
              Sig_sp_r(i,j,n)=real(1.d0/(w(n)-ed-Sigma(n,ick)))
c              Sig_sp_i(i,j,n)=aimag(Sigma(n,ick))
              Sig_sp_i(i,j,n)=aimag(1.d0/(w(n)-ed-Sigma(n,ick)))
            end do
            end do
 ...
 3      format('Enter kx1,ky1,kx2,ky2,offset,nkpoints:  ',$)
     ...
     &             '    ImSigkw')")
        ...
        do ick=1,nkpoints ! nkpoints may be 1
          ...
          do i=1,nf
            c2=0.5*(blint(kx,ky,i)+blint(ky,kx,i))
c            c2=blint(kx,ky,i)
...
c
        ...
        complex(kind) :: z1,z2,z3,z4,Sapprox(Nc),c9
...
c          if(r8.gt.-gamma) r8=-gamma
          if(abs(cmplx(r7,r8)).lt.0.000001) stop'M=0'
          c9=w(n)-1.d0/cmplx(r7,r8)
          if (aimag(c9).gt.-gamma) c9=cmplx(real(c9),-gamma)
          blint=c9
