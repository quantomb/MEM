...
c     chi''(-w) = -chi''(w).  
c
c     This code is special for anomalous single-particle spectra.  Here,
c     the data 
c
c     F(K,tau)=F(K,-tau)  and of course  F(K,tau)=-F(K,tau+beta)
c
c     so that 
c
c     F(K,tau) = = F(K,beta-tau)
c
c     In order to impose this symmetry, A_12(-w) = -A_12(w), so that 
c
c                exp(-tau w) - exp(-(beta-tau) w)
c     K(tau,w) = --------------------------------  Anomalous Fermion
c                       1  + exp(-beta w)   
c
c
c*******************************************************************************
...
c*******************************************************************************
      ...
      integer i1,i2,i,j,ier,ixvgr,nl,run,nf,nuse,
     &        nneg,l, isign
     ...
     &     Bf(nfmax,nfmax),gtau_model(ndatamax),model(nfmax),shift

      double precision, parameter :: pi=3.1415927,zeror=0.0d0
c*******************************************************************************
...
c*******************************************************************************
      ...
      open(unit=12,file='model_shift',status='unknown')
      open(unit=13,file='model_data',status='unknown')
     ...
     &   index(linemc(1:72),'agr').ne.0) ixvgr=1
      ...
      write(6,"('       enter the anomalous shift:  ',$)") 
      read(5,*) shift
...
c
      do i=1,10000
        read(10,*,end=122) w(i),model(i),dw(i)
        write(12,*) w(i),zeror,dw(i)
      end do
 122  nf=i-1
c
...
c     The kernel should reflect the symmetry of the data
     ...
     &          0.5d0*w(j)*(exp(-ta*w(j))-exp(-(beta-ta)*w(j)))
     &         /(1.0d0+exp(-beta*w(j)))
            if(w(j).lt.0.0d0) R(i,j)=
     &          0.5d0*w(j)*(exp((beta-ta)*w(j))-exp(ta*w(j)))
     &         /(1.0d0+exp(beta*w(j)))
          end do
        end do
     ...
     &         /(1.0d0+exp(-beta*w(j)))
      ...
      do i=1,nuse-(1-isign)
        gtau(i) = gtau(i) + shift*sum(R(i,1:nf)*dw(1:nf))
        gtau_model(i) = sum(R(i,1:nf)*model(1:nf)*dw(1:nf))
        write(13,*) tau(i), gtau_model(i)
      end do
      
      if(isign.eq.0.and.w(1).lt.0.0d0) then
        R(nuse,:)=1.0
      else if(isign.eq.0.and.w(1).ge.0.0d0) then
        R(nuse,:)=2.0
      end if
...
c     
      write(96,*) nuse-nneg,nf,run,shift,'  shift'
      if(ixvgr.eq.1) then
