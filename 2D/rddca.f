...
c
     ...
     &                Acurtr(nlmax),Askewr(nlmax),Aprobr(nlmax)
        ...
        real(kind) :: betal,edl,Ul,tprimel
...
c       READ IN THE HEADER DATA (NOTE THE ASSUMED FORM)
 400    read(92,201) linemc
        ...
        if(nfile.eq.1) then
          ...
          do ick=1,Ncw
            write(6,10) ick,Kcx(ick),Kcy(ick)
          end do
          write(6,*) ' icr  Rcx(ick)   Rcy(ick)'
          do icr=1,Nc
            write(6,11) icr,Rcx(icr),Rcy(icr)
          end do
...
c           Now symmetrize the data
     ...
     &                         0.125*gmeas(n,icrequ(icr,iequiv)) 
                end do                                  
              end do                                                    
            end do                      
            if(ick.ne.0) then                           
...
c             factor of 1/Nc is required here)                                  
              do n=1,nl                                                 
                csum=(0.0,0.0)                                          
                do icr=1,Nc                                             
                  csum=csum+exp(ii*Kcx(ick)*Rcx(icr))*                  
     &                      exp(ii*Kcy(ick)*Rcy(icr))*                  
     &                      data1(n,icr)                
                end do                                                  
                store(n)=real(csum)                                     
              end do    
...
c             ds(j,ibin)=abs(store(j))
 ...
 999    continue
        ...
        if(iopt.eq.0.and.isym.eq.0) then ! for G which is not symmetric
          ds(nuse+1,:)=1.0-ds(1,:)       ! Impose G(0+)+G(beta-)=1
          nuse=nuse+1
          sus=0.0
