...
c       
     ...
     &nneg,l
      ...
      integer nfmax,ndatamax,nreal,iisign
      parameter(ndatamax=400,nfmax=1000)
      double precision gtau(ndatamax),tau(ndatamax),data(ndatamax),
     &R(ndatamax,nfmax),Rp(ndatamax,nfmax),w(nfmax),dw(nfmax),     
     2work(2*ndatamax),umat(ndatamax,ndatamax),
     3cov(ndatamax,ndatamax),eigs(ndatamax),
     4umatp(ndatamax,ndatamax),eigsp(ndatamax),
     5Bf(nfmax,nfmax)
      character*72 linemc
      character*10 astr
      integer symques
      ...
      external ssvdc
      double precision range,normalor,r5
...
c
     ...
     1   index(linemc(1:72),' nl').ne.0   ) then
         read(92,*) nl,nreal,run,beta,normalor!number of real part
         temp=1.0/beta
      else
         goto 401
      endif
...
c       check for inconsistencies
      ...
      if(w(1).lt.0.0.and.ikernel.gt.2.and.ikernel.ne.5) then
...
c        The kernel should reflect the symmetry of the data
     ...
     &(1.0-exp(-beta*w(j)))
         ...
         do i=1,nuse
            ta=tau(i)
            do j=1,nf
               if(abs(ta).gt.1.0d-4) then
cc                 R(i,j) = w(j)/(ta**2+w(j)**2)
      ...
      else if (ikernel.eq.6) then  !matsubara freq. kernel(real+img)
         iisign = 0
         do j=1,nf
            do i=1,nreal
               if(abs(tau(i)).gt.1.0d-4) then
                  R(i,j)=  w(j)**2/(tau(i)**2+w(j)**2)
                 !R(i,j)= - w(j)/(tau(i)**2+w(j)**2)
               else
                  R(i,j) = 1.0d0
               endif
            end do
            do i=nreal+1,nuse-(1-iisign)
               R(i,j)= tau(i)*w(j)/(tau(i)**2+w(j)**2)
               !R(i,j)= - tau(i)/(tau(i)**2+w(j)**2)
            end do
         end do
      end if
