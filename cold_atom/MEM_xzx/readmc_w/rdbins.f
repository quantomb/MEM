...
c f77 -r8 -O -o rdbins rdbins.for  
      ...
      integer matsu_flag
      integer j_tmp1,j_tmp2
      parameter(nlmax=200,nbinsmax=400000)
      real*4 beta,tau,dtau,tau_in(nlmax)
      ...
      real*4 PI,normalor,tmp_data(nlmax,6),tmp_w0
...
c     each bin will be skipped.  Typically (now) skip=1
 ...
 14   format('                               enter skip:  ',$)
      ...
      write(6,15) 
 15   format('enter coarse grain size for bin averaging:  ',$)
      read(5,*) naver
...
c
 ...
 400     read(92,201) linemc
         if(index(linemc(1:72),'Results').ne.0 ) then
            read(92,*) nl,nl_bin,beta,normalor
...
cc specify how many matsubara frequencies used
      if(matsu_flag.eq.1) then
         nuse = matsu_freq
      endif
...
c
      ...
      if(matsu_flag.eq.1) then !matsu_freq is the number of real one
         write(9,*) nuse,matsu_freq,nbins,beta,normalor
      else
