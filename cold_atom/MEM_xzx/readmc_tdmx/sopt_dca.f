...
c*******************************************************************************
     ...
     &Kc(2,ick),ickdeg(ick)
          ...
          write(16,*) ' ick  Kcx(ick)   Kcy(ick)    Kcz(ick)  
     &degeneracy'
          do ick=1,Ncw
...
c           Make a coarse-grained model
     ...
     &                  1.0_kind/(w(n) - Epsk - ed - Sigma(n))
            ...
            do n=1,nf
              ...
              if(abs(w(n)).lt.wmax) write(unit+40,"(e12.6,1x,e12.6,
     &1x,e12.6)") w(n),r3,dw(n)
            end do
          else
            ...
            do n=1,nf
              r3=-aimag(Sigma(n))/pi
              if(r3.lt.0.0000001_kind) r3=0.0000001_kind
              if(abs(w(n)).lt.wmax) write(unit,"(e12.6,1x,e12.6,1x,
     &e12.6)") w(n),r3,dw(n)
            end do
          end if
        end if
