...
c        real(kind), parameter :: signswitch=0.1_kind
     ...
     &              sbin(nbinsmax),fills,signswitch,tmpxdd
     ...
     &                             ds(:,:),gsmeas(:,:),data1(:,:)
        integer, allocatable :: indexinproc(:),indexall(:,:)
...
c*******************************************************************************
c         Read in the all bins of Gtau, and try to 
...
c*******************************************************************************
 ...
 444      nproc=nproc+1
          ...
          write(6,*) 'number of binned Gtau    =',i1
          nbinsuse_per_proc=max(1,nbinsuse/nproc)
          allocate(indexinproc(nproc),stat=info)
          allocate(indexall(nproc,nbinsuse_per_proc),stat=info)
          indexall(:,:)  = 0
          indexinproc(:) = 0
...
c         find a bin header
 409         read(92,"(a72)",end=16) linemc
             if(index(linemc(1:72),'bin of  Gtau').ne.0) then
                ...
                ! record index for a proc
                indexinproc(proc+1)=indexinproc(proc+1)+1
                ! record whole index for all proc
                indexall(proc+1,indexinproc(proc+1)) = ibinn
                !readin a bin
                read(92,*) ((gsmeas(j,icr), j=1,nl),icr=1,Nc),smeas 
                sbin(ibinn)=smeas
 ...
 16       continue
          write(6,*) 'done first read '
          rewind(92)
          
!---------------------------------------------------------------------------
! initialize the vacuum term
          indexinproc(:) = 0
...
c             find a bin header
 408         read(92,"(a72)",end=15) linemc
             if(
     &(index(linemc(1:72),'bin of  Chi' ).ne.0).and.(iopt.eq.1).or.
     ...
     &(index(linemc(1:72),'bin of  Gtau').ne.0).and.(iopt.eq.0).or.
     &(index(linemc(1:72),'chi_ddt(n)').ne.0).and.(iopt.eq.8).or.
     &(index(linemc(1:72),'chi_kkt(n)').ne.0).and.(iopt.eq.9).or.
     &(index(linemc(1:72),'chi_eet(n)').ne.0).and.(iopt.eq.10).or.
     &(index(linemc(1:72),'chi_kdt(n)').ne.0).and.(iopt.eq.11) ) then
                read(linemc,"(35x,i20)") proc
                ! locate the index in a proc
                indexinproc(proc+1) = indexinproc(proc+1) + 1
                ! locate the whole index for all proc
                index_proc = indexall(proc+1, indexinproc(proc+1))
                !readin a bin
                ibin=ibin+1
                if(iopt.ge.8) then
                !------------------------------------------------------
     ...
     &,tmpxdd
                ...
                ! if sgn match or not?
                if(abs(smeas-sbin(index_proc)).gt.eps) stop 
     &'sign inconsistent'
                if(ick.ne.0) then                           
                ! Now symmetrize the Green function data
                   data1=0.0
                   if(iopt.ne.7) then
                      do n=1,nl                                         
                        do icr=1,Nc       
...
c                     symmetry of the order parameter
     ...
     &                                 symfacr(icr,jc)*gsmeas(1,jc)
                   ...
                   ! Fourier transform G(X,tau) to obtain G(K,tau).      
                   do n=1,nl                                           
                      csum=(0.0,0.0)                                    
                      do icr=1,Nc                                      
                   ...
                   !---------------------------------------------------
                   if(iopt.ge.8) then
                      ds(nl+2,ibin)=tmpxdd! if iopt>=8, record vacuum
                   endif
                else if(ick.eq.0) then
                   ! calculate the R=0 result (DOS)
                   if(iopt.eq.6) then
                ...
                goto 408
             endif !endif index=Gtau, Chs, Cha ...
 15       continue !enddo nbinuse
          write(6,*) 'done second read '
...
c*******************************************************************************
        ...
        do i=1,nbins
          do j=1,naver
            ...
            !----------------------------------------------------------
            ! for iopt>=8, fillsbin is vacumm value
...
!-----------------------------------------------------------------------
! Subtract the vacumm term for susceptibility of double occupancy
        if(iopt.ge.8) then
           do i = 1, nbins
           do k = 1, nl
              data(k,i)=data(k,i) - dble(Nc)*fillsbin(i)**2/sbin(i)
           enddo
           enddo
        endif
c*******************************************************************************
