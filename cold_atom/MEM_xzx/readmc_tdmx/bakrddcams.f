...
c        real(kind), parameter :: signswitch=0.1_kind
     ...
     &              sbin(nbinsmax),fills,signswitch,tmpxdd,chi_d0
...
c*******************************************************************************
        write(6,"('G(0),CHS(1),CHC(2),CC(3),SC(4),B2(5),XDD(8)?:  ',$)")
        read(5,*) iopt
...
c*******************************************************************************
 ...
 444      nproc=nproc+1
          ...
          allocate(indexall(nproc,nbinuse),stat=info)
          indexall(:,:)  = 0
          indexinproc(:) = 0
...
c             find a bin header
 408          read(92,"(a72)",end=15) linemc
     ...
     &(index(linemc(1:72),'chi_ddtau').ne.0).and.(iopt.eq.8) )
     &        then
                read(linemc,"(35x,i20)") proc
     ...
     &,tmpxdd
                     tmpxdd = gsmeas(1,1)
                     chi_d0 = chi_d0 + tmpxdd
                  else
                     read(92,*) ((gsmeas(j,icr), j=1,nl),icr=1,Nc),smeas
                  endif
! accumulate vacuum term
...
! Subtract the vacumm term for susceptibility of double occupancy
        if(iopt.eq.8) then
           chi_d0 = chi_d0/float(nbins)
           do i = 1, nbins
              do k = 1, nl
                 data(k,i)=data(k,i) - chi_d0**2
              enddo
           enddo
        endif
c*******************************************************************************
