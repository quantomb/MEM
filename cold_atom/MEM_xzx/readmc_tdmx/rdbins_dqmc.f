...
c f77 -r8 -O -o rdbins rdbins.for  
      ...
      integer :: nfiles,nfile,nl_bin,info,nclass
      integer :: icr,isign
      parameter(nlmax=200,nbinsmax=8000)
      real(8) :: beta,tau,dtau
      real(8) ::data(nlmax,nbinsmax),data2(nbinsmax),aver(nlmax)
      real(8), allocatable :: store(:,:)
      real(8) :: sbin(nbinsmax),chibin(nbinsmax)
      real(8) :: normalor,sus1,r1,smeas,chimeas
      real(8) :: ave,adev,sdev,var,skew,curt,chsq,prob,error
      real(8) :: ds(nlmax,nbinsmax)
      real(8) :: PI,tmp_data(nlmax,6)
      parameter(PI=3.1415926535897932354626d0)
      real(8)::signswitch
      parameter(signswitch=0.2d0)
      character*72 linemc
      character*5 chr_tmp
...
! readin file header
 ...
 400     read(92,201) linemc
         if(index(linemc(1:72),'Results').ne.0 ) then
            read(92,*) nl,nclass,beta,normalor
...
! find a bin header
 407        read(92,201,end=410) linemc
            if(index(linemc(1:72),'processor').ne.0) then
               ibin=ibin+1
! readin one bin
               read(92,*) ((store(j,icr),j=1,nl),icr=1,nclass),
     &smeas,chimeas
! transfer data to array ds and do average <X.s>/<s>
               if(isymm.eq.0) then
                  do j=1,nuse
                     ds(j,ibin)=SUM(store(1+skip*(j-1),:))
                     data(j,ibin)=0.0
                  enddo
                  ds(nuse+1,ibin) = smeas
                  ds(nuse+2,ibin) = chimeas
               else
! symmetrize data
                  ds(1,ibin)=SUM(store(1,:))
                  do j=2,nuse
                     ds(j,ibin)=0.5d0*( SUM(store(1+skip*(j-1),:))+
     &SUM(store(1+skip*(nl+1-j),:)) )  ! j --> nl+2-j
                     data(j,ibin)=0.0
                  enddo
                  ds(nuse+1,ibin) = smeas
                  ds(nuse+2,ibin) = chimeas
...
! is not used.
      ...
      do i=1,nbins
         do j=1,naver
            ind=ind+1
            do k=1,nuse+1
               data(k,i)=data(k,i)+ds(k,ind)/dble(naver)
            end do
            sbin(i)  =sbin(i)  + ds(nuse+1,ind)/dble(naver)
            chibin(i)=chibin(i)+ ds(nuse+2,ind)/dble(naver)
         end do
      end do
!
! remove the vacuum contribution <K>**2, <K><D>, or <D>**2, <E>**2
      do i = 1, nbins
      do l = 1, nuse
         data(l,i) = data(l,i)-dble(nclass)*chibin(i)**2/sbin(i)
      enddo
      enddo
!
! normalize the data !!
! For imaginary time case!
      ...
      r1=2.d0/3.d0
      do l=1,nuse
         chimeas = SUM(data(l,1:nbins))
         normalor=normalor+r1*dtau*chimeas
         write(6,*) l, chimeas/dble(nbins)
         r1=2.d0-r1
      enddo
      normalor=normalor/dble(nbins)              ! << s*chi(T) >>
      sus1=SUM(data(nuse+1,1:nbins))/dble(nbins)        ! << s >>
      if(sus1.ge.signswitch) then
         isign = 1
      else
         isign = 0
         write(*,*) 'Woops, sign is too small!'
      endif
!      chi(T) = sus/sus1
      data(nuse+1,1:nbins)=                       ! <s> * <<s*chi(T) >>/<< s >>
     &data(nuse+1,1:nbins)*normalor/sus1
...
c          is one.
!----------------------------------------------------------------------
! If isign=1, we dont use the sign data to calculate the covariance.  
! In this case, we must divide the data by the average sign in a bin.
! Fortunately, both the data for l<=nuse and the sign data have been
! divided by the global average sign <<s>>, so the ratio is precisely 
! what we want; e.g.
!
!       <Gs>/<s> when iopt=0 or 7.
!       or
!       <s chi(tau)><<s>>/(<<s chi(T)>> <s>) 
!       = <s chi(tau)>/ (<s> chi(T)) when iopt>=1.
!----------------------------------------------------------------------
        if(isign.eq.1) then
          do i=1,nbins
             ...
             do k=1,nuse
! data(nuse+1,i) = <s>/<<s>>, and data(k<nuse,i) = <Gs>/<<s>>
! so the line below creates <Gs>/<s>.
                data(k,i)=data(k,i)/data(nuse+1,i)
             end do
          enddo
        end if
!----------------------------------------------------------------------
! If isign=0, then we want to use the sign data in the covariance.
! In this case, we dont divide by <s>.  In this case, data now contains
! (to reiterate):
!
!       <Gs> when iopt=1.
!       or
...
c
      write(9,*) 'Results for nl,nbins,beta,sus,isign'
      write(9,281) nuse,nbins,beta,normalor,isign
      write(9,*) ' '
...
c
 ...
 201  format(a72)
 281  format(x,i5,x,i5,x,e14.7,x,e14.7,x,i3)
      stop
      end
!======================================================================
