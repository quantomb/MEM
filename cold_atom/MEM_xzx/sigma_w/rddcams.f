...
c       Declare local variables
        ...
        integer, parameter :: nbinsmax=80000
        integer :: i,j,ic,k,l,lr,i1,n,nuse,ind,naver,ihisto,nreal,
     ...
     &             ick,icr,iequiv,ios,nproc,proc,iproc
        !the number of Matsubara Frequency we used
        integer :: nuse_matsu
...
c*******************************************************************************
c      
        write(6,"('    How many Matsubara Frequency we use ?:  ',$)")
        read(5,*) nuse_matsu
...
c       plot is generated of each time slice data.
        ...
        write(6,*) '  '
c*******************************************************************************
c       Read in the number of processors
c*******************************************************************************
        write(6,"('                      Enter the data file:  ',$)")
        read(5,*) linemc
        open(unit=13,file=linemc,status='old')
  28    read(13,"(a72)") linemc
        if(index(linemc,'procs').ne.0) then
           read(13,*) nproc, ndim
        else
           goto 28
        endif   
...
c*******************************************************************************
c       icontrol is used to turn on(1) or turn off(0) frequency truncation 
c*******************************************************************************
c        write(6,"('Enter icontrol:',$)")  
c        read(5,*) icontrol
...
c
     ...
     &    .and.(ick.eq.0.or.ickmap(ickdiff(ickpi,ick)).eq.ick)
     &    .or.iopt.ge.1)then
...
c           G(tau)=-G(beta-tau) (i.e. when iopt=7)
            ...
            if(iopt.eq.7) then
              nuse=nuse/2 ! since G(tau=beta/2)==0
            else
              nuse=nuse/2+1
            end if
          end if
...
c           first input file.
            call allocate_1p
            allocate(data(600,nbinsmax),stat=info)
            ...
            allocate(datar(2*nuse+1,nbinsmax),stat=info)
            ...
            allocate(ds(2*nuse+2,nbinsmax),stat=info)
            allocate(gsmeas(2*nuse+2,Nc),stat=info)
            ...
            if(ndim.eq.1) then
              ...
              do i=1,Ncw
                write(6,"(1x,i3,1x,f9.6,2x,f9.6,2x,f9.6)") i,Kc(1,i),
     &Kc(2,i),Kc(3,i)
              end do
              write(6,*) ' icr  Rcx(icr)   Rcy(icr)   Rcz(icr)'
              do icr=1,Nc
                write(6,"(1x,i3,5x,i2,9x,i2,9x,i2)") icr,Rc(1,icr),
     &Rc(2,icr),Rc(3,icr)
              end do
            end if
...
c         For the mastubara frequency MEM the bins number can be very big, 
c         so you may need to increase the maximum il number. 
c*******************************************************************************
c          nproc=0; i1=0; ave=0.0
c          do while (i1.lt.200000)            
c          read(92,"(a72)",end=444) linemc
c          if(index(linemc(1:72),'bin of  Gw').ne.0) then
c            i1=i1+1
c            read(linemc,"(35x,i20)") proc
c            if(proc.gt.nproc) nproc=proc
c            read(92,*) ((gsmeas(j,icr), j=1,2*nuse+2),icr=1,Nc),smeas 
c            ave=ave+smeas
c          end if
c          end do
c 444      nproc=nproc+1
c          rewind(92)
c          write(6,*) 'number of processors used=',nproc
c          nbinsuse_per_proc=max(1,nbinsuse/nproc)
c          write(6,*) 'bins used per processor=',nbinsuse_per_proc
c          write(6,*) 'average sign', ave/float(i1)
...
c             find a bin header
 408          read(92,"(a72)",end=15) linemc
              if(index(linemc(1:72),'bin of  Gw').ne.0.and.iopt.eq.0) then
...
c    are 2*(nuse+1) numbers in one bin 
                  read(92,*,err=408) ((gsmeas(j,icr), j=1,2*nuse),
     &icr=1,Nc),smeas, r1
!                  write(6,*) ((gsmeas(j,icr), j=1,2*nuse),icr=1,Nc)
!                  pause
                  !!! some case smeas may be negative!
                  !smeas=DABS(smeas) 
                  if(ABS(smeas).lt.0.1) then
                    write(6,*) 'sgn too small!',smeas,'ibin=',ibin
                    goto 408 
                  end if      
                  ibin=ibin+1
c                 if(abs(smeas-sbin(ibin)).gt.eps) stop'sign inconsistent'
                  if(ick.ne.0) then                           
c                   Now symmetrize the Green function data
                    ...
                    do n=1,2*nuse                                       
c    mastubara frequency  Greens function G(k,iwn)        
                     do j=1,ngroup
                        ds(n,ibin)=ds(n,ibin) + 
     &gsmeas(n,jc(j))/float(ngroup)   
                     end do 
                    end do    
                    ds(2*nuse+1,ibin)=smeas  ! average sign in a bin
c                    if(smeas.lt.0.9) then
c                       write(6,*) smeas,ibin
c                    end if   
                  elseif(ick.eq.0) then      
c                   calculate the R=0 result (DOS)
                    do n=1,2*nuse
                       do icr=1,Nc
c                                     1     /         \
c    FT from k to r=0.   G(r=0,iwn)= ---Sum | G(k,iwn)|
c                                     Nc    \         /
                          ds(n,ibin)=ds(n,ibin)+gsmeas(n,icr)/real(Nc)  ! i.e. r=0
                       end do
                    end do
                    ds(2*nuse+1,ibin)=smeas  ! average sign in a bin
                  else
 ...
 15        continue
c 420       write(6,*) 'wrong with file','icr=',icr,'nuse=',j,'smeas=',smeas
c           stop   
c 999    continue ! nfile
        nbins=ibin
        if(nbins.gt.nbinsmax) stop 'in rddca.f increase nbinsmax'
c        do i=1,nbins
c         write(6,*) ds(2*nuse+1,i),2*nuse+1
c        end do 
        signa=sum(ds(2*nuse+1,1:nbins))/float(nbins)
c        fills=sum(ds(nl+2,1:nbins))/float(nbins)
        write(6,*) '     total bins=',nbins
c        write(6,*) 'average filling=',fills/signa
        write(6,*) '   average sign=',signa
        if(signa.gt.signswitch) then
...
c            sbin(i)=sbin(i)+ds(2*nuse+1,ind)/float(naver)
c            fillsbin(i)=fillsbin(i)+ds(2*nuse+2,ind)/float(naver)
          end do
c          if(data(2*nuse+1,i).lt.0.9) then
c            write(6,*)(ds(2*nuse+1,k),k=ind-naver,ind)
c          end if        
        end do
...
c*******************************************************************************
        ...
        if(iopt.eq.0.or.iopt.eq.7) then
           sus=sum(data(2*nuse+1,1:nbins))/float(nbins) ! The global average sign <<s>>.
...
c*******************************************************************************
        write(17,"(2x,i4,3x,f7.3,3x,f10.4,3x,f10.4)") ick,beta,sus/sus1,
     &sus2/sus1   ! ick, beta, chi(T), chi(tau=0)
        write(6,*) 'nbins=',nbins
        write(6,*) '   nl=',nl
        write(6,*) '  '
...
c               so the line below creates <Gs>/<s>.
                data(k,i)=data(k,i)/data(2*nuse+1,i)
             end do
          enddo
        end if
...
c This part is for the truncating of mastubara frequency
C        nl=2*nuse+1
C        nuse=min(int(4*beta),nuse)
         !Specify the Matsubara Frequency number we use
        if(nuse.gt.nuse_matsu) nuse=nuse_matsu 

        nuse=2*nuse
        if(isign.eq.0) nuse=nuse+1 ! if <<s>> is small use it in C_ij

        i= -1 ! Matsubara frequency of QMC data start from wn(0) 
...
c         The imaginary and real part of one Greens function has the same mastubara frequency
          if(mod(l-1,2).eq.0) i=i+1
          Aw(l)=wn(i)
          Aerror(l)=error
          Acurt(l)=curt
          Askew(l)=skew
...
!c       Then we reject the data for which the probability falls below Pmin.
        ...
        do l=1,nuse-(1-isign)
         if(mod(l,2).ne.0) then
           if(Aprob(l).gt.Pmin) then
c            Keep the real data
             lr=lr+1
             datar(lr,:)=data(l,:)
             Awr(lr)=Aw(l)
             averr(lr)=aver(l)
...
c       Then we reload the data back into the original arrays.
        ...
        do l=1,nuse
          ...
          Askew(l)=Askewr(l)
          Aprob(l)=Aprobr(l)
c          write(6,*) 'prob=',Aprob(l) 
        end do

c       Now write stuff out to the output data file.
c        
        write(9,*) 'Results for nuse,nreal,run,beta,isign'
        write(9,*) nuse,nreal,nbins,beta,isign
        write(9,*) ' '
...
c23456789012345678901234567890123456789012345678901234567890123456789012
     ...
     &             ' prob'
        ...
        open(unit=11,file='Gw',status='unknown')
        
        write(9,*)'Real Greens function ReGw'    
        do l=1,nreal
         write(9,"(1x,f9.4,1x,e14.7,1x,e14.7,1x,f5.2,1x,f5.2,1x,e12.6)")
     &         Aw(l),aver(l),Aerror(l),Acurt(l),Askew(l),Aprob(l)
          write(11,*) Aw(l),aver(l) 
        end do

        write(11,*) '&&' 

        write(9,*)'Imaginary Greens function ImGw' 
        do l=nreal+1,nuse
         write(9,"(1x,f9.4,1x,e14.7,1x,e14.7,1x,f5.2,1x,f5.2,1x,e12.6)")
     &         Aw(l),aver(l),Aerror(l),Acurt(l),Askew(l),Aprob(l)
          write(11,*) Aw(l),aver(l) 
        ...
        call covar(data,600,nbinsmax,nuse,nbins,aver,9) 
