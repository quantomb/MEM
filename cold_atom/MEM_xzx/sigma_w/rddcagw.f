...
c        real(kind), parameter :: signswitch=0.1_kind
     ...
     &                sbin(nbinsmax),fills,signswitch,g_ns
...
c       set signswitch according to iopt (maybe should be read in??)
        if(iopt.eq.0.or.iopt.eq.7) then
          ...
          signswitch=0.1  ! o_2nly use in desperation
        else
...
c***** For Matsubara Green's Function Gn(k,wn), "nl" should be the number of
c***** Matsubara Frequencies
            read(92,*) nl,nwn,cluster,ndim
c***** Because Gn(k,wn) is complex, we should read in both real and
c***** imaginary parts, thus nuse should be doubled
            nuse = nl*2
            nl_2 = nl*2
          else
           goto 400
...
c           first input file.
            call allocate_1p
            allocate(data(nl_2+1,nbinsmax),stat=info)
            allocate(data2(nbinsmax),stat=info)
            allocate(Atau(nl_2+1),stat=info)
            allocate(Aave(nl_2+1),stat=info)
            allocate(Aerror(nl_2+1),stat=info)
            allocate(Acurt(nl_2+1),stat=info)
            allocate(Askew(nl_2+1),stat=info)
            allocate(Aprob(nl_2+1),stat=info)
            allocate(datar(nl_2+1,nbinsmax),stat=info)
            allocate(averr(nl_2+1),stat=info)
            allocate(Ataur(nl_2+1),stat=info)
            allocate(Aaver(nl_2+1),stat=info)
            allocate(Aerrorr(nl_2+1),stat=info)
            allocate(Acurtr(nl_2+1),stat=info)
            allocate(Askewr(nl_2+1),stat=info)
            allocate(Aprobr(nl_2+1),stat=info)
            allocate(ds(nl_2+2,nbinsmax),stat=info)
            allocate(gsmeas(nl_2,Nc),stat=info)
            allocate(data1(nl_2+1,Nc),stat=info)
            allocate(aver(nl_2+1),stat=info)
            call tables
            if(ndim.eq.2) then
     ...
     &Rc(2,icr),Rc(3,icr)
            ...
            write(6,"('   Nc=',i4,'   Ncw=',i4,'        Enter ick:  ',
     &$)") Nc, Ncw
            read(5,*) ick
          end if
...
c*******************************************************************************
          ...
          do while (i1.lt.1024)            
             ...
             if(index(linemc(1:72),'bin of  Gtau').ne.0) then
                ...
                read(92,*) ((gsmeas(j,icr),j=1,nl_2),icr=1,Nc),smeas
                ave=ave+smeas
             end if
          end do
 444      nproc=nproc+1
...
c         as Gs(X,tau),s or Gn(K,wn),s. Here, Gs(Gn) is the average product of the
...
c         ds(nl+1,ibin) and ds(nl+2,ibin), resp.
          ibinn=ibin  ! not ibin its needed below
          do iproc=0,nproc-1  
            do 16 i=1,nbinsuse
c             find a bin header
 409          read(92,"(a72)",end=16) linemc
              if(index(linemc(1:72),'bin of  Gtau').ne.0) then
                read(linemc,"(35x,i20)") proc
                if(proc+1.gt.nproc) stop 'nproc wrong'
                if(proc.eq.iproc) then !readin a bin
                  ibinn=ibinn+1        ! doesn't start at 0 if ifile>1
c***** Read in both real and imaginary part of Green's function
                  read(92,*) ((gsmeas(j,icr),j=1,nl_2),icr=1,Nc),smeas
                  sbin(ibinn)=smeas
c***** fills=<ns>, for Matsubara frequency, 
c***** G(r=0,\tau=0)=\sum_{iw_n} G(r=0,iw_n)
                  do il= 1, nl_2, 2
                     g_ns = g_ns + gsmeas(il,1)
                  enddo
c                  ds(nl+2,ibinn)=2.0*(smeas-gsmeas(1,1))
                  ds(nl+2,ibinn)=2.0*(smeas-g_ns)
                end if
              else
                goto 409
              endif
 ...
 408          read(92,"(a72)",end=15) linemc
     ...
     &        then
                ...
                if(proc.eq.iproc) then !readin a bin		
                  ibin=ibin+1
                  read(92,*) ((gsmeas(j,icr),j=1,nl_2),icr=1,Nc),smeas
                  if(abs(smeas-sbin(ibin)).gt.eps) stop 
     &'sign inconsistent'
                  if(ick.ne.0) then                           
...
c                     symmetry of the order parameter
     ...
     &halfr*(gsmeas(n,jc)-gsmeas(nl_2-n+2,jc)) 
                          end do                                  
                        end do                                          
                      end do    
                      do icr=1,Nc       
...
c                   Fourier transform G(X,tau) to obtain G(K,tau).      
                    ...
                    ds(nl_2+1,ibin)=smeas  ! average sign in a bin
                  else if(ick.eq.0) then 
c                   calculate the R=0 result (DOS)
                    if(iopt.eq.6) then
                      ds(1:nl_2,ibin)=gsmeas(1:nl_2,icR0) + 
     &gsmeas(1:nl_2,icR1)
                    else
                      ds(1:nl_2,ibin)=gsmeas(1:nl_2,icR0)  ! i.e. r=0
                    end if
                    ds(nl_2+1,ibin)=smeas  ! average sign in a bin
                  else
                    write(6,*) '*****ERROR********'
 ...
 999    continue ! nfile
        ...
        signa=sum(ds(nl_2+1,1:nbins))/float(nbins)
        fills=sum(ds(nl_2+2,1:nbins))/float(nbins)
...
c*******************************************************************************
        ...
        do i=1,nbins
          do j=1,naver
            ...
            sbin(i)=sbin(i)+ds(nl_2+1,ind)/float(naver)
            fillsbin(i)=fillsbin(i)+ds(nl_2+2,ind)/float(naver)
          end do
        end do
...
c*******************************************************************************
        ...
        if(iopt.eq.0.or.iopt.eq.7) then
           data(nuse+1,1:nbins)=data(nl_2+1,1:nbins)  ! load the signs after the last Gs
...
c       Then we reject the data for which the probability falls below Pmin.
        lr=0
        do l=1,nuse-(1-isign)
          if(Aprob(l).gt.Pmin.or.l.eq.1.or.l.eq.nuse.and.iopt.ne.7) then
...
c23456789012345678901234567890123456789012345678901234567890123456789012
     ...
     &             ' prob'
        ...
        do l=1,nuse
          write(9,"(1x,f9.4,1x,e14.7,1x,e14.7,1x,f5.2,1x,f5.2,1x,f7.5)") 
     &         Atau(l),aver(l),Aerror(l),Acurt(l),Askew(l),Aprob(l)
        ...
        call covar(data,nl_2+1,nbinsmax,nuse,nbins,aver,9)
