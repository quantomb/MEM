...
c
c       This code reads Lex's data, and uses only the impure part,
c       the second half of the data.
c
c*******************************************************************************
        use Global
        implicit none
...
c       Declare local variables
     ...
     &             nbins,iopt,isym,nbinsuse,nbinsuse_per_proc,
     &             info,isign,
...
c
     ...
     &    .or.iopt.ge.1)then
c           Use symmetric storage mode if G(tau)=G(beta-tau)
...
c         ds(nl+1,ibin) and ds(nl+2,ibin), resp.
          ...
          do iproc=nproc/2,nproc-1  ! Lex's data, only use impure part
c          do iproc=0,nproc/2         ! Lex's data, only use pure part
            do 16 i=1,nbinsuse        
c             find a bin header
