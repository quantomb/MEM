...
c       some local declarations 
     ...
     &             ik, isk, nkpoints, info, infot, ikx, iky, deg, nfb
        ...
        real(kind) :: r1, r2, r3, kx, ky, kx1, ky1, kx2, 
...
c                    ---      ----<---- k+q  w'+w/2
c               2 pi \   /
c chipp2(Q,w) = ---- |   | dw' A(K+Q,w'+w/2) A(K,-w'+w/2) [f(-w'+w/2)+f(w'+w/2)-1]
c                Nc  /   /
c                    ---
c                     K
c
     ...
     &         (fermi(w(j)-0.5*wb(i),beta)-fermi(w(j)+0.5*wb(i),beta))
          chipp2(i,ick)=chipp2(i,ick)-dw(j)*r1*r3*
     &         (fermi(-w(j)+0.5*wb(i),beta)+fermi(w(j)+0.5*wb(i),beta)-1.0)
          chipp2_d(i,ick)=chipp2_d(i,ick)-dw(j)*gdwave(jck)*gdwave(jck)*r1*r3*
     &         (fermi(-w(j)+0.5*wb(i),beta)+fermi(w(j)+0.5*wb(i),beta)-1.0)
          chipp2_1(i,ick)=chipp2_1(i,ick)-dw(j)*gdwave(jck)*r1*r3*
     &         (fermi(-w(j)+0.5*wb(i),beta)+fermi(w(j)+0.5*wb(i),beta)-1.0)
        end do
        end do
        end do
