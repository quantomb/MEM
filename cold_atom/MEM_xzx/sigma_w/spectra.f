...
c*******************************************************************************
     ...
     &                ky2, offset, res, hall, vx, vyy, dfermi,kplot,
     &                L11, L12, L21, L22
...
c        if ((w(i).le.0.d0).and.(w(i).gt.-4.d0)) then
        write(25,"(5(g12.6,1x))") kplot, -aimag(c1)/pi+(i-1-nf/4)
     &*offset*0.25, w(i), kx, ky
c        endif
...
c            c2     = 0.5*(blint(kx,ky,i)+ blint(ky,kx,i))
c            c2=blint(kx,ky,i)
            r1     = aimag(1.0/(w(i)-ed-r1-c2))        ! G(k,w)
     ...
     &         dw(i)*deg*dfermi*(vx**2)*vyy*(r1)**3
        ...
        do i=1,nf
          ...
          L11=L11+dw(i)*dfermi*tau(i)         !  /    df    i    j-1
          L12=L12+dw(i)*dfermi*tau(i)*w(i)    ! -| dw -- tau(w) w
          L21=L21+dw(i)*dfermi*tau(i)**2      !  /    dw
          L22=L22+dw(i)*dfermi*w(i)*tau(i)**2 ! 
        end do
        write(44,"(f7.4,1x,f10.5,1x,f10.4,4(1x,f10.5))") 
     &        temp,res,hall,L11,L12,L21,L22
