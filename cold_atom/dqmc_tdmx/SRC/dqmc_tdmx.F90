...
contains
 ...
 !--------------------------------------------------------------------!
  ...
  subroutine DQMC_TDMX_Free(TX)
    ...
    !deallocate(TX%dynmod_wkk, TX%dynmod_wkd)
  ...
  subroutine DQMC_TDMX_MeasALL(TX,upt0_eq,up0t_eq,dnt0_eq,dn0t_eq,&
    ...
    do ib = 1, NADJ
       ...
       do jb = 1, NADJ
          ...
          ! connected graph
          value_kk = value_kk - upt0_un(j_neibor,i)*up0t_un(i_neibor,j)&
                              - dnt0_un(j_neibor,i)*dn0t_un(i_neibor,j)
       enddo
       enddo
       ! disconnected graph
       tmp1 = tmp1 - up0t_eq(i_neibor,i) - dn0t_eq(i_neibor,i)
    enddo
    enddo
    ...
    do ib = 1, NADJ
       i_neibor = neibor(ib,i)
       do j = 1, n
          value_kd = value_kd - up0t_eq(i_neibor,i)*G_kk(1,j)*G_kk(2,j)
          value_kd = value_kd - dn0t_eq(i_neibor,i)*G_kk(2,j)*G_kk(1,j)
          value_kd = value_kd - upt0_un(j,i)*up0t_un(i_neibor,j)*G_kk(2,j)
          value_kd = value_kd - dnt0_un(j,i)*dn0t_un(i_neibor,j)*G_kk(1,j)
       enddo
    enddo
    enddo
    ...
    do i= 1, n
       do j = 1, n
          do jb = 1, NADJ
             j_neibor = neibor(jb,j)
             value_dk = value_dk + upt0_un(j_neibor,i)*up0t_un(i,j)*dn0t_eq(i,i)
             value_dk = value_dk + dnt0_un(j_neibor,i)*dn0t_un(i,j)*up0t_eq(i,i)
          enddo
          !<D(T_i)D(0)>
          tmp1 = -up0t_eq(i,i)*G_kk(1,j)-upt0_un(j,i)*up0t_un(i,j)
          tmp2 = -dn0t_eq(i,i)*G_kk(2,j)-dnt0_un(j,i)*dn0t_un(i,j)
          value_dd = value_dd + tmp1*tmp2
       enddo
       value_dk = value_dk + up0t_eq(i,i)*Gsum_kl(1)*dn0t_eq(i,i)
       value_dk = value_dk + dn0t_eq(i,i)*Gsum_kl(2)*up0t_eq(i,i)
    enddo
  ...
  subroutine DQMC_TDMX_EQT(TX,upt0,up0t,dnt0,dn0t,G_kk,Gsum_kl)
    ...
    do j  = 1, n
       do ib = 1, NADJ
          ...
          do jb = 1, NADJ
             ...
             value_kk = value_kk + tmp1*tmp2
             ! connected graph
                value_kk = value_kk - upt0(j_neibor,i)*up0t(i_neibor,j)&
                                    - dnt0(j_neibor,i)*dn0t(i_neibor,j)
          enddo
          !<K(0)D(0)>
          value_kd = value_kd - up0t(i_neibor,i)*up0t(j,j)*dn0t(j,j)
          value_kd = value_kd - dn0t(i_neibor,i)*dn0t(j,j)*up0t(j,j)
          value_kd = value_kd + upt0(j,i)*up0t(i_neibor,j)*dn0t(j,j)
          value_kd = value_kd + dnt0(j,i)*dn0t(i_neibor,j)*up0t(j,j)
       enddo
       !<D(0)K(0)>
       do jb = 1, NADJ
          j_neibor = neibor(jb,j)
          value_dk = value_dk - up0t(i,i)*up0t(j_neibor,j)*dn0t(i,i)
          value_dk = value_dk - dn0t(i,i)*dn0t(j_neibor,j)*up0t(i,i)
          value_dk = value_dk + upt0(j_neibor,i)*up0t(i,j)*dn0t(i,i)
          value_dk = value_dk + dnt0(j_neibor,i)*dn0t(i,j)*up0t(i,i)
       enddo          
       !<D(0)D(0)>
    ...
    do i = 1, n
       do ib = 1, NADJ
          i_neibor = neibor(ib,i)
          Gsum_kl(1) = Gsum_kl(1) - up0t(i_neibor,i)
          Gsum_kl(2) = Gsum_kl(2) - dn0t(i_neibor,i)
       end do
       G_kk(1,i) = -up0t(i,i)
       G_kk(2,i) = -dn0t(i,i)
  ...
  enddo
  write(unit_t,"('#sgn=',1x,E16.8)") TX%sgn
  write(unit_t,"('#<KD>=',1x,E16.8)") DSQRT(ABS(TX%dynmod_e(1,TX%avg)&
                       *TX%dynmod_e(2,TX%avg)*alphax))
  ...
  enddo
  write(unit_t,"('#sgn=',1x,E16.8)") TX%sgn
  write(unit_t,"('#<E>=',1x,E16.8)") TX%dynmod_e(1,TX%avg)&
                                +TX%dynmod_e(2,TX%avg)*alphax
