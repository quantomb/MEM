...
#include "dqmc_include.h"
  ...
  type Hubbard
     ...
     integer  :: nBin                     ! Number of data bins
     integer  :: nIter                    ! Number of iteration of MC
...
contains
  ...
  subroutine DQMC_Hub_Config(Hub, cfg)
    ...
    integer :: seed, nOrth, nWrap, nTry, nBin, nMeas, nIter
    ...
    call DQMC_Hub_Init(Hub, U, t, mu, L, n_t, n_U, n_mu, dtau, HSF, &
         nWarm, nPass, nMeas, nTry, nBin, nIter,seed, nOrth, nWrap, &
  ...
  !---------------------------------------------------------------------!
  subroutine DQMC_Hub_sprng_init(Hub,seed)
  !--------------------------------------------------------------------
  ! subroutine for initialize sprng in simqmc module
  !
  ! SPRNG DECLARE
  ! =============
#define SIMPLE_SPRNG
...
#include "sprng_f.h"
  ...
  ! Execute
  ! =======
    junkPtr = init_sprng(SPRNG_CMRG,seed,SPRNG_DEFAULT)

  end subroutine DQMC_Hub_sprng_init
  !---------------------------------------------------------------------!

  subroutine DQMC_Hub_Init(Hub, U, t, mu, L, n_t, n_U, n_mu, dtau, &
       HSF_IPT, nWarm, nPass, nMeas, nTry, nBin, nIter, seed, nOrth, nWrap, &
       errrate, difflim, gamma, accept, reject, delta1, delta2, HSFtype)
    !
    ! Purpose
    ! =======
    ...
    integer,  intent(in)  :: nMeas, nBin, nWrap, accept, reject, nIter
    ...
    ! Initialize SPRNG random variable generation
    ! call DQMC_Hub_sprng_init(Hub,seed)
    ...
    if (bins .gt. 0) then
       call DQMC_Phy0_Init(Hub%P0, Hub%S, Hub%nBin, Hub%WS)
       call DQMC_Phy2_Init(Hub%P2, Hub%nBin, Hub%S, Hub%WS)
       Hub%meas2 = .true.
    end if
