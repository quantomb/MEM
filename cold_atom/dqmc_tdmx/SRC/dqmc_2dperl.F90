...
contains
  ...
  subroutine DQMC_Comp_2DPerl(IPT, OPT)
    !
    ! Purpose
    ! =======
  ...
  subroutine DQMC_Init_2DPerl(nx, ny, S, IMP)    
    ...
    write(S%name,'(A,I3,A,I3,A,I5)') &
         "2D Periodic Rectangular Lattice (Nx=", &
  ...
  subroutine DQMC_2DPerl_DC_Imp1(n, nx, ny, S, cord)    
    ...
    S%F(1) = n
    write(S%clabel(1),FMT_INTPAR) 0, 0
    ...
    do i = 1, n
       ...
       do j = i+1, n
          ...
          if (L(long,short) .eq. 0) then
             ...
             L(long, short) = idx
             S%D(i,j) = idx
             write(S%clabel(idx),FMT_INTPAR) long-1, short-1
             cord(idx, 1) = long  - 1
             cord(idx, 2) = short - 1
    ...
    do i = 1, n
       ...
       do j = i+1, n
          ...
          if (L(dx,dy) .eq. 0) then
             ...
             L(dx, dy) = idx
             S%D(i,j) = idx
             write(S%clabel(idx),FMT_INTPAR) dx-1, dy-1
