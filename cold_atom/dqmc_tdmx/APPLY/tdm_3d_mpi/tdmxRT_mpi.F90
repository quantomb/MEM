program mpitest
  ...
  type(hubbard) :: Hub
  type(TDMX_RT) :: tm
  integer       :: nx, ny, nz, n, L, nClass, nl
  ...
  real(8),allocatable :: collect_kdt(:,:)!<KD>
  ...
  do i = 1, Hub%nBin
     ...
     if(myrank.eq.0) then
       ...
       do rank = 0, nproc-1
          ...
          write(unit_t,"(6(1X,f12.7))") ((collect_kkt(ic,j),j=1,L),ic=1,nClass)&
           ,collect_sgn(1),collect_vcm(1)

          unit_t = 20000
          write(unit_t,"('#No. bin=',i5,' from processor',i5)") i,rank
          write(unit_t,"(6(1X,f12.7))") ((collect_kdt(ic,j),j=1,L),ic=1,nClass)&
           ,collect_sgn(1),DSQRT(collect_vcm(1)*collect_vcm(2))


          unit_t = 30000
          write(unit_t,"('#No. bin=',i5,' from processor',i5)") i,rank
          write(unit_t,"(6(1X,f12.7))") ((collect_ddt(ic,j),j=1,L),ic=1,nClass)&
           ,collect_sgn(1),collect_vcm(2)

          unit_t = 40000
          write(unit_t,"('#No. bin=',i5,' from processor',i5)") i,rank
          write(unit_t,"(6(1X,f12.7))") ((collect_eet(ic,j),j=1,L),ic=1,nClass)&
