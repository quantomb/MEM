program mpitest
  ...
  type(hubbard) :: Hub
!  type(TDMX)    :: tm
  integer       :: nx, ny, nz, n, L
  integer       :: i, j, k, il, unit_t
  integer       :: ntausk,nClass,nphy
  real          :: s1, s2, dtau
  real(wp)      :: alphax
...
!  allocate(collect_kkt(L))
!  allocate(collect_kdt(L))
!  allocate(collect_ddt(L))
!  allocate(collect_eet(L))
...
!          if(rank.ne.0) call MPI_RECV(collect_kkt,L,MPI_REAL8,rank,itag,MPI_COMM_WORLD,istat,ierr)
!          if(rank.ne.0) call MPI_RECV(collect_kdt,L,MPI_REAL8,rank,itag,MPI_COMM_WORLD,istat,ierr)
!          if(rank.ne.0) call MPI_RECV(collect_ddt,L,MPI_REAL8,rank,itag,MPI_COMM_WORLD,istat,ierr)
!          if(rank.ne.0) call MPI_RECV(collect_eet,L,MPI_REAL8,rank,itag,MPI_COMM_WORLD,istat,ierr)
...
!          do il = 1, L
!             write(unit_t,"(1X,i4,1x,E16.8)") il, collect_kkt(il)
!          enddo
!          write(unit_t,"('#sgn=',1x,E16.8)") collect_sgn(1)
!          write(unit_t,"('#<K>=',1x,E16.8)") collect_vcm(1)
!          write(unit_t,"('#<D>=',1x,E16.8)") collect_vcm(2)
!
!
!          unit_t = 20000
!          write(unit_t,"('#No. bin=',i5,' from processor',i5)") i,rank
!          do il = 1, L
!             write(unit_t,"(1X,i4,1x,E16.8)") il, collect_kdt(il)
!          enddo
!          write(unit_t,"('#sgn=',1x,E16.8)") collect_sgn(1)
!          write(unit_t,"('#<K>=',1x,E16.8)") collect_vcm(1)
!          write(unit_t,"('#<D>=',1x,E16.8)") collect_vcm(2)
...
!          write(unit_t,"('#No. bin=',i5,' from processor',i5)") i,rank
!          do il = 1, L
!             write(unit_t,"(1X,i4,1x,E16.8)") il, collect_ddt(il)
!          enddo
!          write(unit_t,"('#sgn=',1x,E16.8)") collect_sgn(1)
!          write(unit_t,"('#<K>=',1x,E16.8)") collect_vcm(1)
!          write(unit_t,"('#<D>=',1x,E16.8)") collect_vcm(2)
...
!          write(unit_t,"('#No. bin=',i5,' from processor',i5)") i,rank
!          do il = 1, L
!             write(unit_t,"(1X,i4,1x,E16.8)") il, collect_eet(il)
!          enddo
!          write(unit_t,"('#sgn=',1x,E16.8)") collect_sgn(1)
!          write(unit_t,"('#<K>=',1x,E16.8)") collect_vcm(1)
!          write(unit_t,"('#<D>=',1x,E16.8)") collect_vcm(2)
...
!       call MPI_ISEND(collect_kkt,L,    MPI_REAL8,0,itag,MPI_COMM_WORLD,req(8),ierr)
!       call MPI_ISEND(collect_kdt,L,    MPI_REAL8,0,itag,MPI_COMM_WORLD,req(9),ierr)
!       call MPI_ISEND(collect_ddt,L,    MPI_REAL8,0,itag,MPI_COMM_WORLD,req(10),ierr)
!       call MPI_ISEND(collect_eet,L,    MPI_REAL8,0,itag,MPI_COMM_WORLD,req(11),ierr)
!       call MPI_ISEND(collect_vcm,2,    MPI_REAL8,0,itag,MPI_COMM_WORLD,req(12),ierr)
!       call MPI_ISEND(collect_sgn,1,    MPI_REAL8,0,itag,MPI_COMM_WORLD,req(13),ierr)

       call MPI_WAITALL(7,req,istat,ierr)
     endif
