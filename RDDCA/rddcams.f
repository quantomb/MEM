...
c*******************************************************************************
c        if(iopt.eq.0.and.isym.eq.0) then ! for G which is not symmetric
c          ds(nuse+1,:)=1.0-ds(1,:)       ! Impose G(0+)+G(beta-)=1
c          nuse=nuse+1
c          sus=0.0
c        else if(iopt.eq.1.or.iopt.eq.2) then
c          sus=0.0
c          r1=2.0/3.0
c          do l=1,nl
c            sus=sus+r1*dtau*sum(ds(l,:))
c            r1=2.0-r1
c          end do
c          sus=sus/float(nbins)
c          ds=ds/sus
c        end if
c*******************************************************************************
c       Symmetrize the data and calculate the normalization (iopt=2)
...
c                r1=2.0/3.0
c                do l=1,nl
c                   sus=sus+r1*dtau*ds(l,j)
c                   r1=2.0-r1
c                enddo
c                ds(nuse+1,j)=sus*ds(nl+1,j)
c             enddo   
c          end if 
c          sus=sum(ds(nuse+1,:))/float(nbins)
c          ds=ds/sus
c          nuse=nuse+1
