...
c       can be used by the preprocessor, readmcms, of the analytic continuation
...
c       Declare local variables
     ...
     &             ick,icr,iequiv
        ...
        real(kind) :: sus,dsus,Pmin,ave,adev,sdev,var,skew,curt,chsq,
     &                prob,error,tau,dtau,mom1,mom2,h,r1,betal,edl,Ul,
     &                tprimel,smeas        
...
c       iopt=2, read the cluster charge susceptibility
        write(6,"('          Read Gtau (0), CHI (1) CHA (2)?:  ',$)") 
        read(5,*) iopt
        isym=0
...
c           first input file.
            ...
            write(6,*) ' icr  Rcx(ick)   Rcy(ick)'
            do icr=1,Nc
              write(6,"(1x,i3,5x,i2,9x,i2)") icr,Rc(1,icr),Rc(2,icr)
            end do
            write(6,"('   Nc=',i4,'        Enter ick:  ',$)") Nc
            read(5,*) ick
          end if
     ...
     &       mcskip.ne.mcskipl.or.
     &       betal.ne.beta.or.nll.ne.nl.or.measl.ne.meas) then
...
c             readin a bin
     ...
     &          ((gsmeas(j,icr), j=1,nl),icr=1,Nc),smeas
c              smeas=1.d0
c             Now symmetrize the Green function data
c              gsmeas=gsmeas/smeas
              ibin=ibin+1
              data1=0.0
              do n=1,nl                                           
                do icr=1,Nc       
          ...
          do j=1,naver
            ind=ind+1
              do k=1,nl
               ...
               data(nl+1,i)=data(nl+1,i)+ds(nl+1,ind)/float(naver)
          end do
        end do
c*******************************************************************************
c       Now divide by sign
c*****************************************************************************  
...
c*******************************************************************************
        if(iopt.eq.0.and.isym.eq.0) then ! for G which is not symmetric
c          Now divide by sign
           do i=1,nbins
              if(abs(data(nl+1,i)).lt.0.00000001) then
                 write(6,*) 'sgn = ',data(nl+1,i),'G(0)=',data(1,i),
     &    'G(beta)=',data(nl,i), ' bin=',i
                 stop 'sign 0 in bin'
              endif
              do k=1,nl
                 data(k,i)=data(k,i)/data(nl+1,i)
              end do
           enddo


           data(nuse+1,:)=1.0-data(1,:) ! Impose G(0+)+G(beta-)=1
           nuse=nuse+1
           sus=0.0
          ...
          r1=2.0/3.0
          do l=1,nl
            sus=sus+r1*dtau*sum(data(l,:))  
            r1=2.0-r1
          end do
         sus=sus/sum(data(nl+1,:)) !this is chi(T)


c          Now divide by sign
           do i=1,nbins
              if(abs(data(nl+1,i)).lt.0.00000001) stop 'sign 0 in bin'
              do k=1,nl
                 data(k,i)=data(k,i)/data(nl+1,i)
              end do
           enddo
...
c           Keep this set of data
        ...
        write(9,*) nuse,nbins,beta,Nc,ick
        write(9,*) ' '
...
c23456789012345678901234567890123456789012345678901234567890123456789012
        write(9,*) '   tau      Gtau(n)          +/-       curt  skew  prob'
