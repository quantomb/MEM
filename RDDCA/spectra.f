...
c*******************************************************************************
     ...
     &             nkpoints, info, infot, ikx, iky, deg, itunnel
        ...
        real(kind) :: r1, r2, r3, kx, ky, fill, Epsk, kx1, ky1, kx2, 
...
c         First find the coordinates of the cell which contains (kx,ky)
          i1=int((kx*g(1,1)+ky*g(1,2))/(g(1,1)*g(1,1)+g(1,2)*g(1,2))+Nc)
     &      -Nc
          i2=i1+1
          j1=int((kx*g(2,1)+ky*g(2,2))/(g(2,1)*g(2,1)+g(2,2)*g(2,2))+Nc)
     &      -Nc
          j2=j1+1
