...
c       Declare local variables
     ...
     &             ickx,icky,ic,jc,idumtau,idumkx,idumky
     ...
     &             edl,Ul,tprimel,tdprimel,smeas
        ...
        real(kind) :: beta, temp,dummy,dumsgn,average0
...
c            call allocate_1p
            ...
            allocate(gsmeas(nl+1),stat=info)
            allocate(data1(nl+1),stat=info)
            allocate(aver(nl+1),stat=info)
c            call tables
c         write(6,*) ' ick  Kcx(ick)   Kcy(ick), invicRK(ick)'
c            do ick=1,Ncw
c              write(6,10) ick,Kc(1,ick),Kc(2,ick),invicRK(ick)
c            end do
c            write(6,*) ' icr  Rcx(ick)   Rcy(ick) isublat(icr)    ',
c     &        'invicRR(icr) '
c            do icr=1,Nc
c              write(6,11) icr,Rc(1,icr),Rc(2,icr), isublat(icr),
c     &         invicRR(icr) 
c            end do
c 10         format(1x,i3,1x,f9.6,2x,f9.6)
c 11         format(1x,i3,5x,i2,9x,i2)
            write(6,8) Lc            
 8       format('   Lc=',i4,'  Enter kx and ky (integers)',
     &       ' kx<=Lc/2, ky<=kx, (Lc,Lc) for r=0:  ',$)
            read(5,*) ickx, icky
          end if
 
c          mcskipl=mcskip
c          measl=meas
c 401      read(92,201) linemc
c          if(index(linemc(1:72),'warm').ne.0 ) then
c            read(92,*) warm,mcskip,meas,ntr,run,niter,nuse1
c          else
c           goto 401
c          endif
...
402       read(92,201) linemc
          if(index(linemc(1:72),'dtau').ne.0 ) then
            read(92,*) dtau, beta
            temp=1.0/beta
          else
           goto 402
          endif
c          dtau=beta/float(nl)

c          if(abs(ed/U).lt.0.0001.and.abs(tprime).lt.0.001
c     &      .and.abs(tdprime).lt.0.001
c     &       .or.iopt.eq.1.or.iopt.eq.2)then
...
c          if(tprimel.ne.tprime.or.edl.ne.ed.or.Ul.ne.U.or.
c     1       mcskip.ne.mcskipl.or.tdprimel.ne.tdprime.or.
c     2       betal.ne.beta.or.nll.ne.nl.or.measl.ne.meas) then
c            write(6,*) 'This data set is inconsistent with prior set'
c            stop
c          end if
c          end if
...
c           find a bin header
 408        read(92,201,end=410) linemc
       if(index(linemc(1:72),'momentum space D').ne.0.and.iopt.eq.1.or.
     &      index(linemc(1:72),'momentum space G').ne.0.and.iopt.eq.0)
     &      then

            read(92,201) linemc
            if(index(linemc(1:72),'tau ').ne.0) then
               do ic=0,Lc/2
                  do jc=0,ic
                     do l=0,nl-1
                       read(92,*)idumtau,idumkx,idumky,dummy
               if((idumkx.eq.ickx).and.(idumky.eq.icky))
     &         gsmeas(idumtau+1)= dummy
                     enddo
                  enddo
               enddo
               read (92,201) linemc
           ...
           ibin=ibin+1
              ...
              do j=1,nl
                ds(j,ibin)=gsmeas(j)/gsmeas(nl+1)
              end do
              read(92,201) linemc1
              if(index(linemc1(1:72),'Average displacement')
     &         .ne.0.and.iopt.eq.1) then
               read(92,*) average0
c               write(17,*)'aa=', average0
              endif
  ...
  999    continue
        ...
        nbins=ibin
        if((ickx.eq.0).and.(icky.eq.0)) then
        ds=ds- average0**2
      endif
c        ds=ds/500.0
...
c*******************************************************************************
         ...
         if(iopt.eq.0) then
             ...
             do l=1,nl
                ...
                r1=2.0-r1
             enddo
             sus=sus/float(nbins) ! average chi(T)
c             sus1=sum(ds(nl+1,:))/float(nbins) ! average s
             sus2=sum(ds(1,:))/float(nbins)    ! average chi(tau=0)
c             ds(nuse+1,:)=ds(nl+1,:)*sus/sus1  ! s*average chi(T)
             ds=ds/sus
          end if 
c          nuse=nuse+1
...
c*******************************************************************************
        write(17,*) ickx,icky,beta,sus,sus2,nbins
        write(17,*) 'mom1=',sus, '  mom2=',sus2,'ave=',average0
        write(6,*) 'nbins=',nbins
        write(6,*) '   nl=',nl
c        write(6,*) '  '
c        write(6,*) 'warm,mcskip,meas,ntr,run'
c        write(6,*) warm,mcskip,meas,ntr,nbins
c        write(6,*) 'tprime,tdprime,ed,U,beta,sus'
c        write(6,*) tprime,tdprime,ed,U,beta,sus
        nbins=nbins/naver
...
c           Keep this set of data
        ...
        write(9,*) 'Results for nl,nuse,run,beta,Nc,ickx,icky='
        write(9,*)  nl,nuse,nbins,beta,Nc,ickx,icky
        write(9,*) ' '
c        write(9,*) 'warm,mcskip,meas,ntr,run'
c        write(9,*) warm,mcskip,meas,ntr,nbins
        write(9,*) ' '
c       write(9,*) 'tprime,tdprime,ed,U,beta,sus'
c       write(9,203) tprime,tdprime,ed,U,beta,sus
c203    format(2x,f7.3,2x,f6.4,2x,f6.4,2x,f6.3,2x,f6.3,2x,f7.3,2x,f7.4)
c       write(9,*) ' '
        write(9,*) ' '
c        1         2         3         4         5         6         7
